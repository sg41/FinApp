...
async def create_payment(
    ...
    if consent.status != 'authorized':
        raise HTTPException(status_code=400, detail=f"Consent is not authorized. Current status: {consent.status}")
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(payment_url, headers=headers, json=api_body)
...
async def create_internal_transfer(
    ...
    Требует ID счетов из нашей БД и авторизованное согласие на платеж.
    """
    # 1. Найти счета списания и зачисления, проверить принадлежность
    debtor_account = db.query(models.Account).join(models.ConnectedBank).filter(
    ...
    if debtor_account.bank_name != creditor_account.bank_name:
        raise HTTPException(status_code=400, detail="Internal transfers must be between accounts at the same bank.")
    ...
    # 3. Получить конфигурацию банка и токен
    bank_config = db.query(models.Bank).filter(models.Bank.name == debtor_account.bank_name).first()
    if not bank_config:
        raise HTTPException(status_code=500, detail=f"Bank configuration for {debtor_account.bank_name} not found.")
    ...
    # 4. Собрать данные для API запроса из БД
    debtor_account_number = None
    if debtor_account.owner_data and isinstance(debtor_account.owner_data, list) and debtor_account.owner_data:
        debtor_account_number = debtor_account.owner_data[0].get("identification")
